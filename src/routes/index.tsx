...
import { media, departmentImage, doctorImage } from "@/lib/media";
import {
          ...
          {doctors.map((doc: any) => (
            <div key={doc.id} className="card-lift rounded-3xl border border-border bg-card shadow-soft overflow-hidden">
              <Link to="/doctors" className="img-zoom relative aspect-[3/4] block bg-muted">
                <img src={doc.photo_url || doctorImage(doc.name)} alt={`Portrait of ${doc.name}`} loading="lazy" width={900} height={1200} className="h-full w-full object-contain" />
