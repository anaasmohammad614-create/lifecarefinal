...
import { doctorImage } from "@/lib/media";
            ...
            {doctors.map((d: any) => (
              <div key={d.id} className="card-lift rounded-3xl border border-border bg-card shadow-soft overflow-hidden">
                <div className="img-zoom relative aspect-[3/4] bg-muted">
                  <img src={d.photo_url || doctorImage(d.name)} alt={`Portrait of ${d.name}`} loading="lazy" width={900} height={1200} className="h-full w-full object-contain" />
